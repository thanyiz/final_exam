...
class Instance(object):
    def __init__(self, sentence_pack, args):
        ...
        examples = InputExample(
            id=id,
            text_a=sentence_pack["sentence"],
            text_b=None,
            all_label=triple_dict,
...
def convert_examples_to_features(args, train_instances, max_span_length=8):
    ...
    for ex_index, example in enumerate(train_instances):
        ...
        for triple_name in sample["triples"]:
            aspect_span, opinion_span, sentiment = (
                tuple(sample["triples"][triple_name][0]),
                tuple(sample["triples"][triple_name][1]),
                sample["triples"][triple_name][2],
            )
            num_triple += 1
            ...
            for i in range(max_span_length):
                ...
                for j in range(sample["text_length"] - i):
                    ...
                    if (j, i + j) not in opinion_reverse:
                        ...
                        reverse_opinion_label.append(
                            validity2id[opinion_reverse[(j, i + j)]]
                        )
            ...
            for i in range(sample["text_length"]):
                for j in range(i, min(sample["text_length"], i + max_span_length)):
                    ...
                    if (i, j) not in opinion_reverse:
                        ...
                        reverse_opinion_label.append(
                            validity2id[opinion_reverse[(i, j)]]
                        )
        ...
        if args.random_shuffle != 0:
            ...
            for spans_opinion_label_split in spans_opinion_label:
                spans_opinion_label_split_np = np.array(spans_opinion_label_split)[
                    shuffle_ix
                ]
                spans_opinion_label_shuffle.append(
                    spans_opinion_label_split_np.tolist()
                )
            ...
            for reverse_aspect_label_split in reverse_aspect_label:
                reverse_aspect_label_split_np = np.array(reverse_aspect_label_split)[
                    shuffle_ix
                ]
                reverse_aspect_label_shuffle.append(
                    reverse_aspect_label_split_np.tolist()
                )
            reverse_aspect_label = reverse_aspect_label_shuffle
            spans, span_tokens, spans_aspect_label, reverse_opinion_label = (
                spans_np.tolist(),
                span_tokens_np.tolist(),
                spans_aspect_label_np.tolist(),
                reverse_opinion_label_np.tolist(),
            )
        ...
        (
            sample["spans"],
            sample["span tokens"],
            sample["spans_aspect_label"],
            sample["spans_opinion_label"],
        ) = (spans, span_tokens, spans_aspect_label, spans_opinion_label)
        sample["reverse_opinion_label"], sample["reverse_aspect_label"] = (
            reverse_opinion_label,
            reverse_aspect_label,
        )
...
def load_data(args, path, if_train=False):
    ...
    data_instances, aspect_num, num_opinion = convert_examples_to_features(
        args, train_instances=instances, max_span_length=args.max_span_length
    )
    list_instance_batch = []
    for i in range(0, len(data_instances), args.train_batch_size):
        list_instance_batch.append(data_instances[i : i + args.train_batch_size])
    ...
    for ex_index, line in enumerate(lines):
        ...
        for triple in raw_pairs:
            ...
            if word not in triple_dict:
                triple_dict[word] = []
                triple_dict[word] = (
                    [raw_aspect[0], raw_aspect[-1]],
                    [raw_opinion[0], raw_opinion[-1]],
                    sentiment,
                )
            else:
...
class DataTterator(object):
    def __init__(self, instances, args):
        ...
        self.tokenizer = BertTokenizer.from_pretrained(
            args.init_vocab, do_lower_case=args.do_lower_case)
    ...
    def get_batch(self, batch_num):
        ...
        for i, sample in enumerate(self.instances[batch_num]):
            ...
            for spans_aspect2opinion_label in spans_aspect2opinion_labels:
                spans_aspect_labels.append(
                    (i, spans_aspect2opinion_label[0], spans_aspect2opinion_label[1])
                )
            for reverse_opinion2aspect_label in reverse_opinion2aspect_labels:
                reverse_opinion_labels.append(
                    (
                        i,
                        reverse_opinion2aspect_label[0],
                        reverse_opinion2aspect_label[1],
                    )
                )
            (
                bert_tokens,
                ...
                spans_aspect_labels_tensor,
                spans_opinion_tensor,
                reverse_ner_label_tensor,
                reverse_opinion_tensor,
                reverse_aspect_tensor,
            ...
            assert (
                bert_spans_tensor.shape[1]
                == spans_ner_label_tensor.shape[1]
                == reverse_ner_label_tensor.shape[1]
            )
            ...
            sentence_length.append(
                (bert_tokens, tokens_tensor.shape[1], bert_spans_tensor.shape[1])
            )
        ...
        for (
            ...
            if spans_pad_length > 0:
                ...
                reverse_ner_label_tensor = torch.cat(
                    (reverse_ner_label_tensor, mask_pad), dim=1
                )

                opinion_mask_pad = torch.full(
                    [1, num_aspect, spans_pad_length], 0, dtype=torch.long
                )
                spans_opinion_label_tensor = torch.cat(
                    (spans_opinion_label_tensor, opinion_mask_pad), dim=-1
                )
                aspect_mask_pad = torch.full(
                    [1, num_opinion, spans_pad_length], 0, dtype=torch.long
                )
                reverse_aspect_tensor = torch.cat(
                    (reverse_aspect_tensor, aspect_mask_pad), dim=-1
                )
                """对span类似方阵mask"""
            ...
            related_spans_tensor = torch.as_tensor(
                torch.from_numpy(related_spans), dtype=torch.bool
            )
            ...
            if final_tokens_tensor is None:
                ...
                final_spans_opinion_label_tensor = torch.cat(
                    (
                        ...
                        spans_opinion_label_tensor.squeeze(0),
                    ),
                    dim=0,
                )
                ...
                final_reverse_aspect_label_tensor = torch.cat(
                    (
                        ...
                        reverse_aspect_tensor.squeeze(0),
                    ),
                    dim=0,
                )
        ...
        final_spans_opinion_label_tensor = final_spans_opinion_label_tensor.to(
            self.args.device
        )
        final_reverse_ner_label_tensor = final_reverse_ner_label_tensor.to(
            self.args.device
        )
        final_reverse_opinion_tensor = final_reverse_opinion_tensor.to(self.args.device)
        final_reverse_aspect_label_tensor = final_reverse_aspect_label_tensor.to(
            self.args.device
        )
    ...
    def get_input_tensors(
        ...
        spans_aspect_label = [
            [aspect_span[0], start2idx[aspect_span[1]], end2idx[aspect_span[2]]]
            for aspect_span in spans_aspect_label
        ]
        reverse_opinion_label = [
            [opinion_span[0], start2idx[opinion_span[1]], end2idx[opinion_span[2]]]
            for opinion_span in reverse_opinion_labels
        ]
