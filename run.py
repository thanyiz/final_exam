...
def eval(bert_model, step_1_model, step_2_forward, step_2_reverse, dataset, args):
    with torch.no_grad():
        ...
        """前向预测结果"""
        (
            forward_stage1_pred_aspect_result,
            forward_stage1_pred_aspect_with_sentiment,
            forward_stage1_pred_aspect_sentiment_logit,
            forward_stage2_pred_opinion_result,
            forward_stage2_pred_opinion_sentiment_logit,
        ) = ([], [], [], [], [])

        """反向预测结果"""
        (
            reverse_stage1_pred_opinion_result,
            reverse_stage1_pred_opinion_with_sentiment,
            reverse_stage1_pred_opinion_sentiment_logit,
            reverse_stage2_pred_aspect_result,
        ...
        for j in range(dataset.batch_count):
            ...
            bert_features = bert_model(
                input_ids=tokens_tensor, attention_mask=attention_mask
            )

            if j == 0 and args.model_para_test:
                step_1_model.to("cpu")

                flop_step1, para_step1 = profile(
                    step_1_model,
                    inputs=(
            ...
            (
                aspect_class_logits,
                opinion_class_logits,
                spans_embedding,
                forward_embedding,
                reverse_embedding,
                cnn_spans_mask_tensor,
            ) = step_1_model(
                bert_features.last_hidden_state,
                attention_mask,
                bert_spans_tensor,
            ...
            """Batch更新"""
            pred_aspect_logits = torch.argmax(
                F.softmax(aspect_class_logits, dim=2), dim=2
            )
            pred_sentiment_ligits = F.softmax(aspect_class_logits, dim=2)
            pred_aspect_logits = torch.where(
                spans_mask_tensor == 1,
                pred_aspect_logits,
                torch.tensor(0).type_as(pred_aspect_logits),
            )

            reverse_pred_stage1_logits = torch.argmax(
                F.softmax(opinion_class_logits, dim=2), dim=2
            )
            reverse_pred_sentiment_ligits = F.softmax(opinion_class_logits, dim=2)
            reverse_pred_stage1_logits = torch.where(
                spans_mask_tensor == 1,
                reverse_pred_stage1_logits,
            ...
            if torch.nonzero(pred_aspect_logits, as_tuple=False).shape[0] == 0:
                ...
                forward_stage1_pred_aspect_result.append(
                    torch.full_like(spans_aspect_tensor, -1)
                )
                ...
                forward_stage2_pred_opinion_result.append(
                    torch.full_like(spans_opinion_label_tensor, -1)
                )
                forward_stage2_pred_opinion_sentiment_logit.append(
                    torch.full_like(
                        spans_opinion_label_tensor.unsqueeze(-1).expand(
                            -1, -1, len(sentiment2id)
                        ),
                        -1,
                    )
                )
                ...
                pred_aspect_spans = torch.chunk(
                    torch.nonzero(pred_aspect_logits, as_tuple=False),
                    torch.nonzero(pred_aspect_logits, as_tuple=False).shape[0],
                    dim=0,
                )
                pred_span_aspect_tensor = None
                for pred_aspect_span in pred_aspect_spans:
                    batch_num = pred_aspect_span.squeeze()[0]
                    span_aspect_tensor_unspilt_1 = bert_spans_tensor[
                        batch_num, pred_aspect_span.squeeze()[1], :2
                    ]
                    span_aspect_tensor_unspilt = torch.tensor(
                        (
                            batch_num,
                            span_aspect_tensor_unspilt_1[0],
                            span_aspect_tensor_unspilt_1[1],
                        )
                    ).unsqueeze(0)
                    if pred_span_aspect_tensor is None:
                        pred_span_aspect_tensor = span_aspect_tensor_unspilt
                    else:
                        pred_span_aspect_tensor = torch.cat(
                            (pred_span_aspect_tensor, span_aspect_tensor_unspilt), dim=0
                        )
                ...
                (
                    ...
                    spans_mask_tensor,
                    forward_embedding,
                    pred_span_aspect_tensor,
                )

                if j == 0 and args.model_para_test:
                    step_2_forward.to("cpu")

                    flop_step2_f, para_step2_f = profile(
                        step_2_forward,
                        inputs=(
                            all_spans_embedding,
                            all_span_mask,
                ...
                opinion_class_logits, opinion_attention = step_2_forward(
                    all_spans_embedding, all_span_mask, all_span_aspect_tensor
                )
                ...
                forward_stage1_pred_aspect_sentiment_logit.append(pred_sentiment_ligits)
                forward_stage2_pred_opinion_result.append(
                    torch.argmax(F.softmax(opinion_class_logits, dim=2), dim=2)
                )
                forward_stage2_pred_opinion_sentiment_logit.append(
                    F.softmax(opinion_class_logits, dim=2)
                )
            """反向预测"""
            if torch.nonzero(reverse_pred_stage1_logits, as_tuple=False).shape[0] == 0:
                reverse_stage1_pred_opinion_result.append(
                    torch.full_like(reverse_opinion_tensor, -1)
                )
                reverse_stage1_pred_opinion_with_sentiment.append(
                    reverse_pred_stage1_logits
                )
                reverse_stage1_pred_opinion_sentiment_logit.append(
                    reverse_pred_sentiment_ligits
                )
                reverse_stage2_pred_aspect_result.append(
                    torch.full_like(reverse_aspect_label_tensor, -1)
                )
                reverse_stage2_pred_aspect_sentiment_logit.append(
                    torch.full_like(
                        reverse_aspect_label_tensor.unsqueeze(-1).expand(
                            -1, -1, len(sentiment2id)
                        ),
                        -1,
                    )
                ...
                reverse_pred_opinion_spans = torch.chunk(
                    torch.nonzero(reverse_pred_stage1_logits, as_tuple=False),
                    torch.nonzero(reverse_pred_stage1_logits, as_tuple=False).shape[0],
                    dim=0,
                )
                ...
                for reverse_pred_opinion_span in reverse_pred_opinion_spans:
                    ...
                    reverse_opinion_tensor_unspilt = bert_spans_tensor[
                        batch_num, reverse_pred_opinion_span.squeeze()[1], :2
                    ]
                    reverse_opinion_tensor_unspilt = torch.tensor(
                        (
                            batch_num,
                            reverse_opinion_tensor_unspilt[0],
                            reverse_opinion_tensor_unspilt[1],
                        )
                    ).unsqueeze(0)
                    if reverse_span_opinion_tensor is None:
                        ...
                        reverse_span_opinion_tensor = torch.cat(
                            (
                                ...
                                reverse_opinion_tensor_unspilt,
                            ),
                            dim=0,
                        )
                ...
                (
                    ...
                    reverse_bert_embedding,
                    reverse_attention_mask,
                    reverse_spans_embedding,
                    reverse_span_mask,
                ) = stage_2_features_generation(
                    bert_features.last_hidden_state,
                    attention_mask,
                ...
                if j == 0 and args.model_para_test:
                    ...
                    flop_step2_r, para_step2_r = profile(
                        step_2_reverse,
                        inputs=(
                            reverse_spans_embedding,
                            reverse_span_mask,
                ...
                reverse_aspect_class_logits, reverse_aspect_attention = step_2_reverse(
                    reverse_spans_embedding,
                    reverse_span_mask,
                    all_reverse_opinion_tensor,
                )
                ...
                reverse_stage2_pred_aspect_sentiment_logit.append(
                    F.softmax(reverse_aspect_class_logits, dim=2)
                )
        ...
        aspect_result, opinion_result, apce_result, pair_result, triplet_result = (
            metric.score_triples()
        )
        ...
        logger.info(
            "aspect precision: {}\taspect recall: {:.8f}\taspect f1: {:.8f}".format(
                aspect_result[0], aspect_result[1], aspect_result[2]
            )
        )
        logger.info(
            "opinion precision: {}\topinion recall: {:.8f}\topinion f1: {:.8f}".format(
                opinion_result[0], opinion_result[1], opinion_result[2]
            )
        )
        logger.info(
            "APCE precision: {}\tAPCE recall: {:.8f}\tAPCE f1: {:.8f}".format(
                apce_result[0], apce_result[1], apce_result[2]
            )
        )
        logger.info(
            "pair precision: {}\tpair recall: {:.8f}\tpair f1: {:.8f}".format(
                pair_result[0], pair_result[1], pair_result[2]
            )
        )
        logger.info(
            "triple precision: {}\ttriple recall: {:.8f}\ttriple f1: {:.8f}".format(
                triplet_result[0], triplet_result[1], triplet_result[2]
            )
        )
...
def train(args):
    ...
    training_param_optimizer = [
        ...
        {
            "params": [p for n, p in step_1_param_optimizer],
            "lr": args.task_learning_rate,
        },
        {
            "params": [p for n, p in forward_step2_param_optimizer],
            "lr": args.task_learning_rate,
        },
        {
            "params": [p for n, p in reverse_step2_param_optimizer],
            "lr": args.task_learning_rate,
        },
    ]
    ...
    if args.mode == "train":
        ...
        tot_loss = 0
        tot_kl_loss = 0
        best_aspect_f1, best_opinion_f1, best_APCE_f1, best_pairs_f1, best_triple_f1 = (
            0,
            0,
            0,
            0,
        ...
        for i in range(args.epochs):
            ...
            for j in tqdm.trange(trainset.batch_count):
                ...
                step_2_opinion_class_logits, opinion_attention = step2_forward_model(
                    all_spans_embedding, all_span_mask, all_span_aspect_tensor
                )
                step_2_aspect_class_logits, aspect_attention = step2_reverse_model(
                    reverse_spans_embedding,
                    reverse_span_mask,
                    all_reverse_opinion_tensor,
                )
            ...
            if triplet_result[2] > best_triple_f1 and triplet_result[2] > 0.55:
                model_path = (
                    args.model_dir + args.dataset + "_" + str(triplet_result[2]) + ".pt"
                )
                state = {
        ...
        logger.info(
            "best aspect epoch: {}\tbest aspect precision: {:.8f}\tbest aspect recall: {:.8f}\tbest aspect f1: {:.8f}".format(
                best_aspect_epoch,
        ...
        logger.info(
            "best opinion epoch: {}\tbest opinion precision: {:.8f}\tbest opinion recall: {:.8f}\tbest opinion f1: {:.8f}".format(
                best_opinion_epoch,
        ...
        logger.info(
            "best APCE epoch: {}\tbest APCE precision: {:.8f}\tbest APCE recall: {:.8f}\tbest APCE f1: {:.8f}".format(
                best_APCE_epoch, best_APCE_precision, best_APCE_recall, best_APCE_f1
            )
        )
        logger.info(
            "best pair epoch: {}\tbest pair precision: {:.8f}\tbest pair recall: {:.8f}\tbest pair f1: {:.8f}".format(
                best_pairs_epoch, best_pairs_precision, best_pairs_recall, best_pairs_f1
            )
        )
        logger.info(
            "best triple epoch: {}\tbest triple precision: {:.8f}\tbest triple recall: {:.8f}\tbest triple f1: {:.8f}".format(
                best_triple_epoch,
...
def main():
    ...
    parser.add_argument(
        "--model_dir", type=str, default="savemodel/", help="model path prefix"
    )
    ...
    parser.add_argument(
        "--bert_feature_dim", default=768, type=int, help="feature dim for bert"
    )
    parser.add_argument(
        "--do_lower_case",
        default=True,
    ...
    parser.add_argument(
        "--dataset_path",
        default="./datasets/ASTE-Data-V2-EMNLP2020/",
    ...
    parser.add_argument(
        ...
        default="14lap",
        type=str,
        choices=["14lap", "14res", "15res", "16res"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="number of block in generating spans",
    )

    # 是否对相关span添加分离Loss
    parser.add_argument("--kl_loss", default=True)
    parser.add_argument(
        "--kl_loss_weight", type=int, default=0.5, help="weight of the kl_loss"
    )
    parser.add_argument(
        "--kl_loss_mode",
        type=str,
        default="KLLoss",
        choices=["KLLoss", "JSLoss", "EMLoss, CSLoss"],
        help="选择分离相似Span的分离函数, KL散度、JS散度、欧氏距离以及余弦相似度",
    )
    # 是否使用测试中的筛选算法
    parser.add_argument(
        "--Filter_Strategy", default=True, help="是否使用筛选算法去除冲突三元组"
    )
    # 已被弃用    相关Span注意力
    parser.add_argument("--related_span_underline", default=False)
    parser.add_argument(
        "--related_span_block_num",
        type=int,
        default=1,
